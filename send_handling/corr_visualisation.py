...
plot.vert_subplots_2df(scatter, rolled, keys, colour1, colour2, 'report_figs/weather.pdf', trim=True, ylab=ylabels)
...
plot.vert_subplots_2df(scatter, rolled, keys, colour1, colour2, 'report_figs/corrected.pdf', trim=True, ylab=ylabels)
...
plot.vert_subplots_1df_compare(rolled, ['CT_RATE_MOD', 'CT_RATE_UNMOD'], ['CT_RATE_MOD_CORR', 'CT_RATE_UNMOD_CORR'],
                               colour1, colour2, 'report_figs/compare_corr_uncorr.pdf', trim=True, ylab=ylabels)

fig, axes = plt.subplots(1, 2, sharey=True, figsize=(6, 4))
axes[0].hist(rolled['CTS_MOD_CORR'].values, bins=51, color=colour2, density=True)
axes[1].hist(rolled['CTS_UNMOD_CORR'].values, bins=51, color=colour2, density=True)
axes[0].set_ylabel('Probability (%)')
axes[0].set_xlabel('Moderated Neutron \n Counts)')
axes[1].set_xlabel('Unmoderated Neutron \n Counts$)')
plt.tight_layout()
plt.savefig('report_figs/corr_hist.pdf')
plt.show()
